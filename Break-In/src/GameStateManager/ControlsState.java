...
public class ControlsState extends GameState {
	...
	public void draw(Graphics2D g){
		...
		g.setFont(title);
		int length2 = (int) g.getFontMetrics().getStringBounds("Controls", g).getWidth();
		g.drawString("Controls",(GamePanel.WIDTH - length2)/2,62);
		...
		g.setColor(Color.WHITE);
		g.setFont(text);
		int length1 = (int) g.getFontMetrics().getStringBounds("-Move With Arrow Keys-", g).getWidth();
		int length5 = (int) g.getFontMetrics().getStringBounds("-Serve Ball With Space-", g).getWidth();
		int length3 = (int) g.getFontMetrics().getStringBounds("-Pause With Escape-", g).getWidth();
		int length4 = (int) g.getFontMetrics().getStringBounds("-Select With Enter-", g).getWidth();
		g.drawString("-Move With Arrow Keys-", (GamePanel.WIDTH-length1)/2, 80);
		g.drawString("-Serve Ball With Space-", (GamePanel.WIDTH-length5)/2,95);
		g.drawString("-Pause With Escape-",(GamePanel.WIDTH-length3)/2,110);
		g.drawString("-Select With Enter-",(GamePanel.WIDTH-length4)/2,125);
