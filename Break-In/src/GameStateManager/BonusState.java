...
public class BonusState extends GameState {
	...
	private ArrayList<Brick_Bonus>Bonus;
	
	public BonusState(GameStateManager gsm){
		...
		ran = new Random();
		currentChoice = 0;
		title = new Font("Blocked",Font.PLAIN,14);
		text = new Font("Blocked",Font.PLAIN,10);
		running = true;
		paused = false;
		win = false;
		lose = false;
	...
	public void init() {
		try{
			...
			text2 = scFont.deriveFont(Font.PLAIN,8);
			...
			Bonus = new ArrayList<Brick_Bonus>();
	...
	public void keyPressed(int k){
		...
		if(paused){
			if(k == KeyEvent.VK_ENTER){
				...
				if(currentChoice == 1){
					gsm.setState(GameStateManager.BONUSSTATE);
				}
				if(currentChoice == 2){
	...
	private void addBricks(){
		...
		for(int i = 0; i< bonusPos.length;i++){
			Bonus.add(new Brick_Bonus(bonusPos[i][0],bonusPos[i][1]));
		}
	}
	...
	private void brickCollision(){
		...
		for(int i = 0;i < Bonus.size();i++){
			Rectangle r2 = Bonus.get(i).getTop();
			Rectangle r3 = Bonus.get(i).getBottom();
			Rectangle r4 = Bonus.get(i).getLeft();
			Rectangle r5 = Bonus.get(i).getRight();
			...
			if(r1.intersects(r2)){
				...
				score = score+20;
				b.multDy(-1);
				if(Bonus.get(i).getVisible() == false){
